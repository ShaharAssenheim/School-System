...
/**
 * The Class ReqEditingCell for event when click on current cell.
 */
class ReqEditingCell extends TableCell<regrequest, String> {
	...
	/** The list. */
	ObservableList<regrequest> list;
    
    /**
     * Instantiates a new ReqEditingCell.
     *
     * @param list the list of requests
     */
    public ReqEditingCell(ObservableList<regrequest> list) {
    	this.list = list;
        ...
        if (!isEmpty()) {
        	try {
        		...
        		if(getItem().equals((list.get(i).getRequestCol())))
        				{
							screen.SchDir_ExpStdReg(list.get(i));
			        		break;
        				}
        		}
			} catch (IOException e) {
